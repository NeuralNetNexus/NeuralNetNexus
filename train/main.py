...
def train():
    ...
    if response.status_code == requests.codes.ok:
        ...
        requests.patch(f"http://backend-service/projects/{project_id}/splits/{job_completion_index}/logs", json={"logs": f"Downloading dataset {dataset_name}.zip"})
        ...
        requests.patch(f"http://backend-service/projects/{project_id}/splits/{job_completion_index}/logs", json={"logs": f'Error occurred while downloading the dataset. Status code: {response.status_code}. The module will restart.'})
    ...
    for dataset in dataset_collection:
        ...
        for j, hp in enumerate(hps_iter):
            ...
            for k, model in enumerate(model_collection):
                ...
                print(f"==========  TRAIN  ==========")
                requests.patch(f"http://backend-service/projects/{project_id}/splits/{job_completion_index}/logs", json={"logs": "# ==================== Train ==================== #"})
                ...
                for epoch_i in range(hp["epochs"]):
                    ...
                    requests.patch(f"http://backend-service/projects/{project_id}/splits/{job_completion_index}/logs", json={"logs": "Epoch: {}/{}".format(epoch_i+1, hp["epochs"])})
                    ...
                    requests.patch(f"http://backend-service/projects/{project_id}/splits/{job_completion_index}/logs", json={"logs": log})
                    ...
                    try:
                        ...
                        requests.patch(f"http://backend-service/projects/{project_id}/splits/{job_completion_index}/metrics", json=data)
                        sio.emit('trainingMetrics', data)
                    except:
...
if __name__ == '__main__':
    ...
    try:
        requests.patch(f"http://backend-service/projects/{project_id}/logs", json={"logs": f"Starting training #{job_completion_index+1} process."})
        sio.emit('projectLogs', {'projectId': project_id, 'logs': f"Starting training #{job_completion_index+1} process."})
        train()
        requests.patch(f"http://backend-service/projects/{project_id}/logs", json={"logs": f"Completed training #{job_completion_index+1} process."})
        sio.emit('projectLogs', {'projectId': project_id, 'logs': f"Completed training #{job_completion_index+1} process."})
